...
void afficher_file(FILES*F){
    if(F->tete==F->queue)
        ...
        int i=F->queue-1;
        printf("Les activites retenues sont : \n\n");
        while(i>=F->tete){
...
FILES *choix_d_activites_recursif(int *s, int *f, int i, int n){
	int m = i+1;
	while(m<=n && s[m] < f[i]){ //trouver premiere activite de S_(i,n+1)
		m++;
	}
	if(m<=n)
...
int main(int argc, char **argv){
	...
	printf("Le nombre maximal d'activites possibles a realiser est de %d\n\n",m);
	int *Couleurs = coloration_salles(s,f,N);
	for(i=0;i<N;i++){
		printf("L'activite %d se deroulera dans la salle %d\n",i+1,Couleurs[i]);
	}
