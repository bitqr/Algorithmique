...
int main(int argc, char **argv)
{
	...
	ordonnancement_taches(A, P, 8);
	printf("L'ordonnancement optimal est affiché ainsi (les nombres	indiquent les tâches qui sont exécutées, dans l'ordre, de gauche à droite :\n\n");
	...
	printf("|\n\nLe coût de l'ordonnancement optimal est de %f.\n\n", (double)(sd / 8.0));
	...
	while (L != NULL)
	{
		...
		if (j1 == 0)
			printf("Aucune tâche exécutée des dates %d à %d\n", t1, t2);
		else
			printf("La tâche %d executée des dates %d à %d\n", j1, t1, t2);
		t1 = t2;
	}
	printf("\n\n");
