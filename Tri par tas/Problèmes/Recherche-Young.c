...
int recherche_young(int Y[M][N],int m, int n,int cle){
    ...
    int j = 0;
    if(Y[0][0]>cle){
                    printf("L'element %d ne figure pas dans ce tableau de Young\n\n", cle);
                    return -1;
                    }
    if(Y[0][0]==cle){
                    printf("L'element %d  figure a la ligne 1 a la colonne 1\n\n",cle);
                    return 0;
                    }
    while(i<m){
               if(Y[i][n]<cle)
               ...
               if(Y[i][j]==cle){
                                printf("L'element %d figure a la ligne %d a la colonne %d\n\n\n",cle,i+1,j+1);
                                return i*n+j;
                                }
               }
    printf("L'element %d ne figure pas dans ce tableau de Young\n\n", cle);
