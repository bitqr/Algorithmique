...
int defiler2(FILES *F)
{
    ...
    else
    {
        ...
        printf("%d a été supprimé en tête \n\n", x);
...
int supprimer_en_queue(FILES *F)
{
    if (F->tete == F->queue)
    {
        printf("File vide\n\n");
    ...
    if (F->queue == 0)
    {
        F->queue = F->longueur - 1;
    }
    else
    {
        F->queue--;
    }
    int x = F->tableau[F->queue];
    printf("%d a été supprimé en queue\n\n", x);
    return x;
}
